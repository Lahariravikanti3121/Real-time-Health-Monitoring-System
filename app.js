...
let state = {
    ...
    selectedId: null,
    view: 'dashboard',
    charts: { bpm: null, spo2: null, temp: null, bp: null },

chartData: { 
  bpm: Array(30).fill(null), 
  spo2: Array(30).fill(null),
  temp: Array(30).fill(null),
  bpSys: Array(30).fill(null),
  bpDia: Array(30).fill(null)
},
...
function renderDoctorLayout() {
  ...
  <!-- ✅ HEART RATE -->
  <div class="bg-white dark:bg-slate-800/60 rounded-xl border p-6 shadow-sm">
    <div class="flex justify-between mb-4">
      <h3 class="text-slate-500">Heart Rate</h3>
      <span class="text-2xl font-bold text-blue-500" id="val-bpm">--</span>
    </div>
    <div class="h-64 w-full"><canvas id="chart-bpm"></canvas></div>
  </div>

  <!-- ✅ SPO2 -->
  <div class="bg-white dark:bg-slate-800/60 rounded-xl border p-6 shadow-sm">
    <div class="flex justify-between mb-4">
      <h3 class="text-slate-500">SpO₂</h3>
      <span class="text-2xl font-bold text-teal-400" id="val-spo2">--</span>
    </div>
    <div class="h-64 w-full"><canvas id="chart-spo2"></canvas></div>
  </div>

  <!-- ✅ TEMPERATURE (NEW) -->
  <div class="bg-white dark:bg-slate-800/60 rounded-xl border p-6 shadow-sm">
    <div class="flex justify-between mb-4">
      <h3 class="text-slate-500">Temperature</h3>
      <span class="text-2xl font-bold text-orange-500" id="val-temp">--</span>
    </div>
    <div class="h-64 w-full"><canvas id="chart-temp"></canvas></div>
  </div>

  <!-- ✅ BLOOD PRESSURE (NEW) -->
  <div class="bg-white dark:bg-slate-800/60 rounded-xl border p-6 shadow-sm">
    <div class="flex justify-between mb-4">
      <h3 class="text-slate-500">Blood Pressure</h3>
      <span class="text-sm font-bold text-purple-500" id="val-bp">-- / --</span>
    </div>
    <div class="h-64 w-full"><canvas id="chart-bp"></canvas></div>
  </div>
...
<div class="bg-white dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 p-6 shadow-sm">
  <div class="flex justify-between mb-4">
    <h3 class="text-slate-500 dark:text-slate-400 font-medium">Blood Pressure</h3>
    <span class="text-sm font-bold text-purple-500" id="val-bp">-- / --</span>
  </div>
  <div class="h-64 w-full">
...
function initCharts() {
  const common = { 
    ...
    scales: { 
      x: { display: false }, 
      y: { 
        grid: { color: state.isDark ? '#334155' : '#e2e8f0', tickLength: 0 }, 
  ...
  if (ctxBpm) {
    const grad = ctxBpm.getContext('2d').createLinearGradient(0, 0, 0, 400);
    grad.addColorStop(0, 'rgba(59, 130, 246, 0.5)');
    grad.addColorStop(1, 'rgba(59, 130, 246, 0)');
    state.charts.bpm = new Chart(ctxBpm, { 
      type: 'line', 
      data: { 
        labels: Array(30).fill(''), 
        datasets: [{ data: state.chartData.bpm, borderColor: '#3b82f6', backgroundColor: grad, borderWidth: 3, fill: true }] 
      }, 
      options: { ...common, scales: { ...common.scales, y: { min: 40, max: 180, grid: common.scales.y.grid } } } 
    });
  }
  ...
  if (ctxSpo2) {
    const grad = ctxSpo2.getContext('2d').createLinearGradient(0, 0, 0, 400);
    grad.addColorStop(0, 'rgba(45, 212, 191, 0.5)');
    grad.addColorStop(1, 'rgba(45, 212, 191, 0)');
    state.charts.spo2 = new Chart(ctxSpo2, { 
      type: 'line', 
      data: { 
        labels: Array(30).fill(''), 
        datasets: [{ data: state.chartData.spo2, borderColor: '#2dd4bf', backgroundColor: grad, borderWidth: 3, fill: true }] 
      }, 
      options: { ...common, scales: { ...common.scales, y: { min: 85, max: 100, grid: common.scales.y.grid } } } 
    });
  }

  // ✅ TEMPERATURE (SAME STYLE)
  const ctxTemp = document.getElementById('chart-temp');
  if (ctxTemp) {
    const grad = ctxTemp.getContext('2d').createLinearGradient(0, 0, 0, 400);
    grad.addColorStop(0, 'rgba(249, 115, 22, 0.5)');
    grad.addColorStop(1, 'rgba(249, 115, 22, 0)');
    state.charts.temp = new Chart(ctxTemp, {
      type: 'line',
      data: { labels: Array(30).fill(''), datasets: [{ data: state.chartData.temp, borderColor: '#f97316', backgroundColor: grad, borderWidth: 3, fill: true }] },
      options: { ...common, scales: { ...common.scales, y: { min: 34, max: 40, grid: common.scales.y.grid } } }
    });
  }

const ctxBp = document.getElementById('chart-bp');
if (ctxBp) {
  const ctx = ctxBp.getContext('2d');

  const gradSys = ctx.createLinearGradient(0, 0, 0, 400);
  gradSys.addColorStop(0, 'rgba(139, 92, 246, 0.5)');
  gradSys.addColorStop(1, 'rgba(139, 92, 246, 0)');

  const gradDia = ctx.createLinearGradient(0, 0, 0, 400);
  gradDia.addColorStop(0, 'rgba(236, 72, 153, 0.5)');
  gradDia.addColorStop(1, 'rgba(236, 72, 153, 0)');

  state.charts.bp = new Chart(ctxBp, {
    type: 'line',
    data: {
      labels: Array(30).fill(''),
      datasets: [
        {
          data: state.chartData.bpSys,
          borderColor: '#8b5cf6',
    ...
    options: {
      ...
      scales: {
        ...common.scales,
        y: { min: 60, max: 180, grid: common.scales.y.grid }
      }
    }
  });
}
...
socket.on('vital_update', (data) => {
  const mini = document.getElementById(`mini-bpm-${data.id}`);
  if (mini) mini.innerText = Number(data.bpm).toFixed(0);

  if (state.selectedId === data.id) {
    // ✅ Update numbers
    document.getElementById('val-bpm').innerText = Number(data.bpm).toFixed(0);
    document.getElementById('val-spo2').innerText = Number(data.spo2).toFixed(0) + '%';

    if (document.getElementById('val-temp')) {
      document.getElementById('val-temp').innerText = Number(data.temp).toFixed(1) + ' °C';
    }

    if (document.getElementById('val-bp')) {
      document.getElementById('val-bp').innerText =
        `${Number(data.bpSys).toFixed(0)} / ${Number(data.bpDia).toFixed(0)}`;
    }

    // ✅ BPM
    updateChart(state.charts.bpm, state.chartData.bpm, data.bpm);

    // ✅ SPO2
    updateChart(state.charts.spo2, state.chartData.spo2, data.spo2);

    // ✅ TEMP
    if (state.charts.temp) {
      updateChart(state.charts.temp, state.chartData.temp, data.temp);
    }

    // ✅ ✅ ✅ FINAL GUARANTEED BP GRAPH UPDATE ✅ ✅ ✅
    if (state.charts.bp) {
      state.chartData.bpSys.push(Number(data.bpSys));
      state.chartData.bpSys.shift();

      state.chartData.bpDia.push(Number(data.bpDia));
      state.chartData.bpDia.shift();

      state.charts.bp.data.datasets[0].data = state.chartData.bpSys;
      state.charts.bp.data.datasets[1].data = state.chartData.bpDia;

      state.charts.bp.update();
    }
  }
});
...
function updateChart(chart, dataArray, value, datasetIndex = 0) {
...
function startDataStream() {
  ...
  window.simInterval = setInterval(() => {
    state.patients.forEach(p => {
      let old = p.currentVitals?.bpm || 70;
      let newV = old + (Math.random() - 0.5) * 5;
      if (newV > 160) newV = 155;
      if (newV < 50) newV = 55;
      ...
      const vitals = {
        ...
        bpSys: Math.floor(Math.random() * 40) + 110,
        bpDia: Math.floor(Math.random() * 30) + 70
      };

      fetch(`${API_URL}/vitals/${p._id}`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
